...
实现实时库存，提升订单确认速度，提升OTA排名。

酒店在OTA上的排名，是与订单确认速度相关的。订单确认快会给客人带来更好的用户体验，从而提升OTA排名。

未做直连时，酒店通过EBK后台管理库存，但EBK显示的库存通常不准，确认真实库存会消耗时间，降低订单确认速度。直连后，OTA库存与实际库存实时关联，订单确认速度会大幅度提升。
